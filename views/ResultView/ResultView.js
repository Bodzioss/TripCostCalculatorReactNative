...
function ResultView({ passengers }) {
  const getSumOfFuelCosts = (passengers) => {
    return passengers.reduce((sum, passenger) => sum + passenger.fuelCost, 0);
  };

  const getSumOfPassengerAdditionalCosts = (passenger) => {
    return passenger.additionalCosts.reduce(
      (sum, additionalCost) => sum + additionalCost.price,
      0
    );
  };
  ...
  const getSumOfAdditionalCosts = (passengers) => {
    return passengers.reduce(
      (sum, passenger) => sum + getSumOfPassengerAdditionalCosts(passenger),
      0
    );
  };
        ...
        <View style={styles.content}>
          ...
          <View style={styles.passengersList}>
            {passengers.map((passenger) => {
              return <ResultCard key={passenger.id} passenger={passenger} />;
            })}
          </View>
          ...
          <View style={styles.summaryWrapper}>
            ...
            <View style={styles.summaryContent}>
              ...
              <View style={styles.label}>
                ...
                <Text style={styles.labelPrice}>
                  {sumOfAdditionalCosts.toFixed(2)} zł
                </Text>
              </View>
              ...
              <View style={[styles.label, styles.sumLabel]}>
                ...
                <Text style={styles.sumLabelPrice}>
                  {allCosts.toFixed(2)} zł
                </Text>
              </View>
            </View>
          </View>
